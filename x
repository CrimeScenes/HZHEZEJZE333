...
local predictionFactor = 0.15  -- A factor to control how much to predict the target's future position
...
-- Predicitive aiming for moving or jumping targets
local function predictTargetPosition(target, deltaTime)
    local humanoid = target.Character and target.Character:FindFirstChild("Humanoid")
    if humanoid then
        local velocity = humanoid.RootPart.Velocity
        local futurePosition = target.Character.HumanoidRootPart.Position + velocity * deltaTime * predictionFactor
                            ...
                            if tool and tool:IsA("Tool") then
                                if not isIgnoringKnife() then
                                    ...
                                    if shootFunction and shootFunction:IsA("RemoteEvent") then
                                        ...
                                        -- Predict target's future position for moving or jumping players
                                        local predictedPosition = predictTargetPosition(TargetPlayer, 0.1)  -- Predict with a 0.1 second delay
                                        local mouseX, mouseY = Camera:WorldToViewportPoint(predictedPosition)
